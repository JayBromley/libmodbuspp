 ...
 * along with the libmodbuspp Library; if not, see
 ...
 */
template <typename T, Endian e = EndianBig> class Data {
public:
  static_assert((sizeof(T) >= 2 && (sizeof(T) % 2) == 0),
                "Bad Data typename !");
   ...
   */
  Data(const T &t) : m_value(t), m_endian(e) { updateRegisters(); }
   ...
   */
  std::size_t size() const { return sizeof(m_value); }

  /**
   * @brief Array of Modbus registers corresponding to the T value
   */
  std::array<uint16_t, sizeof(T) / 2> &registers() { return m_registers; }
  ...
  void swap(T &v) {
    ...
    case EndianBigLittle: // GHEFCDAB: bytes Big, word Little : swap words
      v = swapWords(v);
      break;
    case EndianLittleBig: // BADCFEHG: bytes Little, word Big : swap bytes of
                          // words
   ...
   * For debugging purpose.
   */
  static void print(const T &v) { print((const uint8_t *)&v, sizeof(v)); }
...
public:
   ...
   * @brief  Overload of the reference operator on the T value
   */
  operator DataEnum &() { return m_value; }

  /**
   * @overload
   */
  operator const DataEnum &() const { return m_value; }

  /**
   * @brief Access to the DataEnum value
   */
  DataEnum &value() { return m_value; }

  /**
   * @overload
   */
  const DataEnum &value() const { return m_value; }

  /**
   * @brief Overload of the pointer operator on the DataEnum value
   */
  DataEnum *operator&() { return &m_value; }

  /**
   * @overload
   */
  const DataEnum *operator&() const { return &m_value; }
 ...
 *
 * Parses @b str interpreting its content as a T number, which is returned as a
 * value of type T. T may be uint16_t, uint32_t, uint64_t, int16_t, int32_t,
...
template <typename T> void strToT(T &v, const std::string &str, int base = 0) {
  ...
  if (idx < str.size()) {
    ...
    throw std::invalid_argument("Unable to convert " + str +
                                " to arithmetic value");
  }
}
} // namespace Modbus
